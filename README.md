...
## フロントエンド
### Vue3.0
* 全面的にSPA（SinglePageApplication)として実装
  * 全ての通信をAjaxで行うことでページ遷移のないアプリケーションを実現
  * Vue.jsのSFCを採用することで可読性を向上

## サーバーサイド
### ウィザード形式のユーザー登録
* セッションを利用した複数テーブルへのユーザー登録実装
* deviseコントローラーをマニュアル設定
...
# 工夫したポイント
### 全ての通信をAjaxで
効率的にAjaxのコードを実装したかったのでJavaScriptのモダンなフレームワークVue.jsを独学。
昨年秋に発表されたVue3を学習したためQiitaなどに解説記事が少なく、参考書と公式リファレンスを読み込みました。
またCSRF-Tokenなど、Railsのヘルパーメソッドが自動で行っていたことを自分で設定しなければいけないため、
結果としてRailsについてより深く学習する事ができました。
### フロントに必要な情報をjsonで送信する
RailsのサーバーサイドレンダリングではHTMLファイルに直接全てのデータを挿入できたが（例えばcurrent_user）、
